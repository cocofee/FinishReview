...
def test_project_and_runtime_versions_match_release_version():
    ...
    assert project_version.group(1) == "0.2.3"
...
def test_windows_executable_version_matches_release_version():
    ...
    assert 'StringStruct("FileVersion", "0.2.3")' in version_info
    assert 'StringStruct("ProductVersion", "0.2.3")' in version_info
    assert "filevers=(0, 2, 3, 0)" in version_info
    assert "prodvers=(0, 2, 3, 0)" in version_info
