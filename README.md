...
上方“时间胶卷”内提供常驻“判读记录”条：按录像时间排列本场在相机 1 确认的
运动员及待补录标记，切换运动员、筛选组别和切换录像文件后仍保留。点击记录
可回到保存的判读位置；相邻记录独立显示，重启后从赛事判读日志恢复。
...
连续浏览。“回到机位 1”定位当前判读时间；换号码、重新确认和新录像归档不会
自动把胶卷拉回前面。胶卷仅解码可见区域及少量邻近图片，缓存有固定上限。
胶卷画面随区域高度放大，卡片宽度按原画面比例调整，保留完整画面。
可拖动胶卷与名单之间的分隔线调节高度，或点击“放大胶卷 / 还原布局”。
...
全场已判号码在时间胶卷和下方判读记录条显示。

“本屏已检查”手动标记完整显示且已加载的缩略图所代表的时间范围，录像缺口、
不可用文件、加载失败和裁切在屏幕边缘的图片不计入；“取消本屏检查”可撤销。
