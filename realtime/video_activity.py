    ...
    def run(self) -> None:
        ...
        try:
            ...
            while frame_index <= end_frame and not self._stop.is_set():
                ...
                progress = min(
                    99,
                    int((frame_index - start_frame) * 100 / total_frames),
                )
...
class ActivityTimelineWidget(QWidget):
    ...
    def set_target_position(self, position_ms: int | None) -> None:
        self._target_ms = -1 if position_ms is None else int(position_ms)
        self.update()

    def set_visible_range(self, start_ms: int, end_ms: int) -> None:
        self._visible_range = (
            min(int(start_ms), int(end_ms)),
            max(int(start_ms), int(end_ms)),
        )
        self.update()

    def set_analysis_state(self, state: str) -> None:
        self._analysis_state = str(state)
        self.update()

    def set_analysis_progress(self, progress: int) -> None:
        self._analysis_progress = max(0, min(100, int(progress)))
        self.update()

    def _x_for_position(self, position_ms: int) -> int:
        span = max(1, self._end_ms - self._start_ms)
        return int(
            (self._end_ms - int(position_ms))
            * max(1, self.width() - 1)
            / span
        )
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.LeftButton and self._end_ms > self._start_ms:
            visual_ratio = max(
                0.0, min(1.0, event.x() / max(1, self.width() - 1))
            )
            # Review advances from right to left: earlier arrivals are on the
            # right and later arrivals are on the left, matching the filmstrip.
            time_ratio = 1.0 - visual_ratio
            self.position_selected.emit(
                int(
                    round(
                        self._start_ms
                        + time_ratio * (self._end_ms - self._start_ms)
                    )
                )
            )
            event.accept()
    ...
    def paintEvent(self, event) -> None:
        ...
        if self._analysis_progress < 100:
            # Analysis runs from earlier to later time, which is visually
            # right-to-left. Shade the not-yet-analyzed area on the left.
            pending_width = int(
                self.width() * (100 - self._analysis_progress) / 100
            )
            painter.fillRect(
                1,
                1,
                max(0, pending_width - 1),
                self.height() - 2,
                QColor(71, 85, 105, 95),
            )
        if self._visible_range is not None and self._end_ms > self._start_ms:
            visible_start, visible_end = self._visible_range
            left = self._x_for_position(visible_end)
            right = self._x_for_position(visible_start)
            painter.fillRect(
                left,
                1,
                max(1, right - left),
                self.height() - 2,
                QColor(59, 130, 246, 38),
            )
            painter.setPen(QPen(QColor(96, 165, 250, 150), 1))
            painter.drawRect(left, 1, max(1, right - left), self.height() - 3)
        ...
        if self._start_ms <= self._target_ms <= self._end_ms:
            x = self._x_for_position(self._target_ms)
            painter.setPen(QPen(QColor("#ef4444"), 2))
            painter.drawLine(x, 1, x, self.height() - 2)
        if self._start_ms <= self._current_ms <= self._end_ms:
            x = self._x_for_position(self._current_ms)
            painter.setPen(QPen(QColor("#ffb020"), 2))
            painter.drawLine(x, 1, x, self.height() - 2)
        painter.setPen(QColor("#d5e3ec"))
        painter.drawText(6, 14, "画面活动 · 右→左")
